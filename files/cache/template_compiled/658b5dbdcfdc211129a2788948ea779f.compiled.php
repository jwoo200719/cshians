...
<form action="" method="post"><input type="hidden" name="error_return_url" value="<?php echo htmlspecialchars(getRequestUriByServerEnviroment(), ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>" /><input type="hidden" name="act" value="<?php echo $__Context->act ?>" /><input type="hidden" name="mid" value="<?php echo $__Context->mid ?>" /><input type="hidden" name="vid" value="<?php echo $__Context->vid ?>" />
	<table id="memberList" class="x_table x_table-striped x_table-hover">
		<caption>
			<a href="<?php echo getUrl('filter_type', '', 'page', '') ?>"<?php if($__Context->filter_type==''){ ?> class="active"<?php } ?>><?php echo $__Context->lang->cmd_show_all_member;
if($__Context->filter_type==''){ ?>(<?php echo $__Context->total_count ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo getUrl('filter_type', 'super_admin', 'page', '') ?>"<?php if($__Context->filter_type==super_admin){ ?> class="active"<?php } ?>><?php echo $__Context->lang->cmd_show_super_admin_member;
if($__Context->filter_type==super_admin){ ?>(<?php echo $__Context->total_count ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo getUrl('filter_type', 'enable', 'page', '') ?>"<?php if($__Context->filter_type==enable){ ?> class="active"<?php } ?>><?php echo $__Context->lang->approval;
if($__Context->filter_type==enable){ ?>(<?php echo $__Context->total_count ?>)<?php } ?></a>
			<i>|</i>
			<a href="<?php echo getUrl('filter_type', 'disable', 'page', '') ?>"<?php if($__Context->filter_type==disable){ ?> class="active"<?php } ?>><?php echo $__Context->lang->denied;
if($__Context->filter_type==disable){ ?>(<?php echo $__Context->total_count ?>)<?php } ?></a>
			<div class="x_pull-right x_btn-group">
				<a class="x_btn x_btn-inverse" href="<?php echo getUrl('', 'module', 'admin', 'act', 'dispMemberAdminInsert') ?>"><?php echo $__Context->lang->msg_new_member ?></a>
		...
		<thead>
			<tr>
				...
				<th scope="col" class="nowr"><a href="<?php echo getUrl('', 'module', 'admin', 'act', 'dispMemberAdminList', 'sort_index', 'regdate', 'sort_order', ($__Context->sort_order == 'asc') ? 'desc' : 'asc') ?>"><?php echo $__Context->lang->signup_date;
if($__Context->sort_index == 'regdate'){ ?> <?php if($__Context->sort_order=='asc'){ ?><em>▲</em><?php };
if($__Context->sort_order != 'asc'){ ?><em>▼</em><?php };
} ?></a></th>
				<th scope="col" class="nowr"><a href="<?php echo getUrl('', 'module', 'admin', 'act', 'dispMemberAdminList', 'sort_index', 'last_login', 'sort_order',  ($__Context->sort_order == 'asc') ? 'desc' : 'asc') ?>"><?php echo $__Context->lang->last_login;
...
if($__Context->sort_order != 'asc'){ ?><em>▼</em><?php };
		...
		<tbody>
			<?php if($__Context->member_list&&count($__Context->member_list))foreach($__Context->member_list as $__Context->no=>$__Context->member_info){ ?><tr>
				...
				<td class="nowr">
					<a href="#popup_menu_area" class="member_<?php echo $__Context->member_info['member_srl'] ?>" title="Info"><?php echo getEncodeEmailAddress($__Context->member_info['email_address']) ?></a>
				</td>
				...
				<td class="nowr" title="<?php echo zdate($__Context->member_info['regdate'], 'Y-m-d H:i:s') ?>"><?php echo zdate($__Context->member_info['regdate'], 'Y-m-d') ?></td>
				<td class="nowr" title="<?php echo zdate($__Context->member_info['last_login'], 'Y-m-d H:i:s') ?>"><?php echo zdate($__Context->member_info['last_login'], 'Y-m-d') ?></td>
				<td><?php echo $__Context->member_info['group_list'] ?>&nbsp;</td>
				<td class="nowr"><a href="<?php echo getUrl('', 'module', 'admin', 'act', 'dispMemberAdminInsert', 'member_srl', $__Context->member_info['member_srl']) ?>"><?php echo $__Context->lang->inquiry ?>/<?php echo $__Context->lang->cmd_modify ?></a></td>
	...
	<input type="hidden" name="xe_validator_id" value="modules/member/tpl/1" />
		...
		<div class="x_modal-body">
			...
			<div class="x_control-group _moveTarget" hidden>
				...
				<?php if($__Context->group_list&&count($__Context->group_list))foreach($__Context->group_list as $__Context->key=>$__Context->val){ ?><label for="g<?php echo $__Context->val->group_srl ?>" class="x_inline"><input type="checkbox" name="groups[]" id="g<?php echo $__Context->val->group_srl ?>" value="<?php echo $__Context->val->group_srl ?>"/> <?php echo $__Context->val->title ?></label><?php } ?>
			</div>
			<div class="x_control-group _moveTarget" hidden>
