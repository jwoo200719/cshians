...
echo sprintf($__Context->lang->rate, $__Context->item->package_start) ?></span></span>
					...
					<p class="x_btn-group" style="text-align:right">
						...
						<?php if($__Context->item->current_version && $__Context->item->avail_remove){ ?><a class="x_btn" href="<?php echo getUrl('act','dispAutoinstallAdminUninstall','package_srl',$__Context->item->package_srl) ?>"><i class="x_icon-remove"></i> <?php echo $__Context->lang->cmd_delete ?></a><?php } ?>
...
if(!in_array($__Context->key, array('mid', 'vid', 'act', 'page'))){ ?><input type="hidden" name="<?php echo $__Context->key ?>" value="<?php echo $__Context->val ?>" /><?php }} ?>
		<input type="search" name="search_keyword" required title="Search" value="<?php echo htmlspecialchars($__Context->search_keyword, ENT_COMPAT | ENT_HTML401, 'UTF-8', false) ?>">
